...
export async function getCapabilitiesSection(variant: PromptVariant, context: SystemPromptContext): Promise<string> {
	...
	const browserCapabilities = context.mcpBrowserAvailable
		? `\n- You have access to a full browser automation suite via the "cura-browser" MCP server (Playwright MCP). You may be connected to the user's REAL Chrome browser via the Cura Browser Extension, with their logged-in sessions, cookies, and open tabs. Use these tools to interact with websites, fill forms, click buttons, navigate pages, and take screenshots.
\t- **Workflow (OBSERVE FIRST)**: FIRST call \`browser_snapshot\` to see the current page state. Then call \`browser_tab_list\` to see all open tabs. If the current page is relevant to the task, interact with it directly. If you need a different page, THEN use \`browser_navigate\`. Always call \`browser_snapshot\` after any navigation or interaction to see the updated state.
\t- **Key tools**: browser_navigate, browser_snapshot, browser_click, browser_type, browser_hover, browser_select_option, browser_drag, browser_press_key, browser_screenshot, browser_tab_list, browser_tab_new, browser_tab_close, browser_go_back, browser_go_forward, browser_wait, browser_file_upload, browser_handle_dialog, browser_pdf_save, browser_console_messages, browser_resize, browser_network_requests.
\t- **MCP API preference**: If a non-browser MCP server can handle the request (e.g., Gmail MCP for email, Notion MCP for notes, Slack MCP for messages), prefer the API tool over navigating to the website in the browser. API tools are faster and more reliable.
\t- For example, to check what the user is looking at: call browser_snapshot first to see the current page → if they're on LinkedIn, interact directly → if they need eBay, use browser_navigate to go to ebay.com → browser_snapshot to see the page → browser_click/browser_type to interact.
